...
exports.checkCCAvailable = checkCCAvailable;
...
function detectClaudeCliPath() {
    ...
    if (isWindows) {
        // Windows: 清理路径，移除回车符
        try {
            const result = (0, child_process_1.execSync)("where claude", { encoding: "utf-8" });
            // 处理 Windows 行结束符 \r\n
...
async function executeChat(options) {
    ...
    if (isWindows && normalizedCliPath.toLowerCase().endsWith('.cmd')) {
        // 尝试找到 cli.js 路径
        const cmdDir = path_1.default.dirname(normalizedCliPath);
        console.log(`[CC] cmdDir: "${cmdDir}"`);
        const cliJsPath = path_1.default.join(cmdDir, 'node_modules', '@anthropic-ai', 'claude-code', 'cli.js');
        console.log(`[CC] cliJsPath: "${cliJsPath}"`);
        if (fs_1.default.existsSync(cliJsPath)) {
    ...
    try {
        ...
        for await (const sdkMessage of (0, claude_code_1.query)(queryOptions)) {
            // 提取 session_id
            if (sdkMessage &&
...
//# sourceMappingURL=cc-bridge.js.map